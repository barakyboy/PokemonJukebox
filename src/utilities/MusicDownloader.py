...
class MusicDownloader:
    """
    A class that is responsible for safely downloading mp3 files
    """
    MP3_DIR = os.getenv('MP3_DIR')  # the path to the directory housing mp3 files
    ...
    MUSIC_FILE_FORMAT = 'mp3'  # the format in which music is saved

    def download_youtube_link(self, link: str):
        """
        Downloads a youtube link, and saves it as a music file. Returns its absolute poth.
        Throws up TooManyFIlesError if there are too many files in MP3_DIR
        :param link: a youtube link.
        :return: the absolute path of the downloaded link
        """

        # check size of directory
        files = [f for f in os.listdir(self.MP3_DIR) if os.path.isfile(os.path.join(self.MP3_DIR, f))]
        ...
        # download video file
        out_file = video.download(output_path=self.MP3_DIR)

        # change to music format
        for i in range(self.MAX_VIDS_IN_DIR):

            candidate_filename = os.path.join(self.MP3_DIR, f'{self.TEMPLATE_VID_NAME}{i}.{self.MUSIC_FILE_FORMAT}')
